...
for ix = 1:length(x)
    ...
    try
        if isempty(linespec{ix})
            hltemp = plot(x{ix}, y{ix},'LineWidth',1.5);
        else
            hltemp = plot(x{ix}, y{ix}, linespec{ix},'LineWidth',1.5);
        end
    catch
        close(htemp);
...
for iln = 1:nline
    hl(iln) = line(xl{iln}, yl{iln}, 'marker', marker{iln}, 'linestyle', lnsty{iln}, 'linewidth', 1.5,'color', lncol{iln}, 'parent', hax);
end
